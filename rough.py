...
'''def addrecco():
    ...
    discount=int(input("Enter the discount available -:"))
    dop=int(input("Enter the purchase date -:"))
    query="insert into stock values({},'{}',{},{},{},'{}')".format(itemcode,itemname,qty,price,discount,dop)
...
'''def searchreco():
        ...
        query="Select * from stock where s="+str(s)
        mycursor.execute(query)
        result=mycursor.fetchone()
...
'''def searchreco():
    itemn=(input("Enter Item name for searching -:"))
    query="Select * from stock where itemn='%n"%itemn
    mycursor.execute(query)
    result=mycursor.fetchone()
    if result!=None:
        print(result)
